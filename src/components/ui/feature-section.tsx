  ...
  return (
    <div className={cn("p-4 sm:p-8 md:p-12", className)}>
      <div className="max-w-7xl mx-auto w-full">
        <h2 className={cn("text-2xl sm:text-3xl md:text-4xl lg:text-5xl mb-8 sm:mb-10 text-center text-white", titleClassName)}>
          {title}
        </h2>
        ...
        <div className="flex flex-col-reverse md:grid md:grid-cols-2 gap-6 md:gap-10 items-center"> {/* Changed flex-col to flex-col-reverse */}
          ...
          <div
            className={cn(
              "w-full md:order-2 relative", // Added w-full
              imageHeight
            )}
          ...
          >
            <AnimatePresence mode="sync">
              {features.map((feature, index) => (
                <motion.div
                  key={index}
                  className={`absolute inset-0 overflow-hidden rounded-lg ${index === currentFeature ? "block" : "hidden"}`}
